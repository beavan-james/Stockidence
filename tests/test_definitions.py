...
def test_sensor_batches_run_requests_per_tick(tmp_path):
    """Bulk enqueues must not stampede providers: each tick stages at most
    SENSOR_BATCH_SIZE requests (oldest first); the rest stay pending."""
    from datetime import datetime, timedelta, timezone

    from dagster import DagsterInstance, build_sensor_context

    from stockidence.definitions import (
        IngestEngine, SENSOR_BATCH_SIZE, ticker_request_sensor)
    from stockidence.storage import Warehouse

    wh = Warehouse(tmp_path / "test.duckdb")
    wh.init_schema()
    base = datetime.now(timezone.utc)
    queued = ["MSFT", "NVDA", "GOOGL", "META", "KO"]
    for k, ticker in enumerate(queued):
        wh.request_ticker(ticker, requested_at=base + timedelta(seconds=k))
    ...
    def _ping():
        with partition_loading_context(dynamic_partitions_store=instance):
            evaluation = ticker_request_sensor.evaluate_tick(
                build_sensor_context(
                    instance=instance,
                    resources={"engine": IngestEngine(wh)},
                    repository_def=_defs.get_repository_def(),
                ))
        return [rr.partition_key for rr in evaluation.run_requests]

    first = _ping()
    assert first == queued[:SENSOR_BATCH_SIZE]
    assert set(wh.pending_ticker_requests()) == set(queued[SENSOR_BATCH_SIZE:])

    second = _ping()
    assert second == queued[SENSOR_BATCH_SIZE:2 * SENSOR_BATCH_SIZE]
    assert len(_ping()) >= 1  # queue keeps draining across ticks
